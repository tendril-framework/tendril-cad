...
from tendril.config import CAD_LIBRARY_FUSION
from tendril.config import CAD_LIBRARY_PRIORITY
...
class MaterialsLibraryManager(object):
    ...
    def _load_libraries(self):
        logger.debug("Loading Materials library modules from {0}".format(self._prefix))
    ...
    def __getattr__(self, item):
        if item == "create_manual_entry":
            return self._libraries['manual'].create_manual_entry
    ...
    def regenerate(self):
        for name, library in iteritems(self._libraries):
            logger.info("Regenerating Materials library '{0}'".format(name))
            library.regenerate()
        self._generate_index()

    def _generate_index(self):
        self.index = {}
        if not CAD_LIBRARY_FUSION:
            self.index = self._libraries[CAD_LIBRARY_PRIORITY[0]].index

        for lname in CAD_LIBRARY_PRIORITY:
    ...
    def get_material(self, ident, get_all=False):
        ...
        raise self.nosymbolexception(
            'Material {0} not found in fused library'.format(ident))

    @property
    def nosymbolexception(self):
        return self._exc_classes['MaterialNotFound']
